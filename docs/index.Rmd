...
What about the mutant S38T? Some proteins are the same as previous samples but also
...
Interestingly it did not standout in any of the individual analysis.
...
pid3 %>% 
   ...
   geom_text_repel(aes(label=ifelse((abs(l2.tarcon) > 0.9 & p.value.tarcon <= 0.1), Gene, '')),
   ...
   theme_bw() +
   xlab("log2(H2A.Z nucleosome / control)") + 
   ylab("-log10(p.value)") +
...
pid3 %>%
   mutate(across(where(is.numeric), round, 3)) %>%
   filter(l2.tarcon > 0.9, p.value.tarcon < 0.1) %>%
